...
class BillCreditNotesSortBy(str, Enum, metaclass=utils.OpenEnumMeta):
    r"""The field on which to sort the Bill Credit Notes"""
...
class BillCreditNotesSortTypedDict(TypedDict):
    by: NotRequired[BillCreditNotesSortBy]
    r"""The field on which to sort the Bill Credit Notes"""
    direction: NotRequired[SortDirection]
    r"""The direction in which to sort the results"""


class BillCreditNotesSort(BaseModel):
    by: Annotated[
        Annotated[
            Optional[BillCreditNotesSortBy], PlainValidator(validate_open_enum(False))
        ],
    ...
    def serialize_by(self, value):
        if isinstance(value, str):
            try:
                return models.BillCreditNotesSortBy(value)
            except ValueError:
                return value
        return value
